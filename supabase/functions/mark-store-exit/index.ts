...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (asText(recordValue(schedule, "fecha")) !== ecuadorDateKey()) {
      return jsonResponse({ error: "Solo puedes marcar salida de hoy." }, 409);
    }
    ...
    if (recordValue(existing, "salida_en")) {
    ...
    const { data: attendance, error: updateError } = await adminClient
      .from(tableAttendance)
      .update({ salida_en: now, actualizado_en: now })
      .eq("horario_id", horarioId)
      .is("salida_en", null)
      .select("*")
      .maybeSingle();
    ...
    console.error("mark-store-exit error:", error);
